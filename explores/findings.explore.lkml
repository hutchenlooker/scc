...
explore: findings_log {
  ...
  join: findings__finding__files {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__files}) as findings__finding__files ;;
    relationship: one_to_many
  }
  join: findings__finding__processes {
    view_label: "Findings: Finding Processes"
    sql: LEFT JOIN UNNEST(${findings_log.finding__processes}) as findings__finding__processes ;;
    relationship: one_to_many
  }
  join: findings__finding__indicator__uris {
    view_label: "Findings: Finding Indicator Uris"
    sql: LEFT JOIN UNNEST(${findings_log.finding__indicator__uris}) as findings__finding__indicator__uris ;;
    relationship: one_to_many
  }
  ...
  join: findings__finding__containers {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__containers}) as findings__finding__containers ;;
    relationship: one_to_many
  }
  join: findings__finding__compliances__ids {
    view_label: "Findings: Finding Compliances Ids"
    sql: LEFT JOIN UNNEST(${findings_log.finding__compliances__ids}) as findings__finding__compliances__ids ;;
    relationship: one_to_many
  }
  join: findings__finding__indicator__domains {
    view_label: "Findings: Finding Indicator Domains"
    sql: LEFT JOIN UNNEST(${findings_log.finding__indicator__domains}) as findings__finding__indicator__domains ;;
    relationship: one_to_many
  }
  join: findings__finding__iam_bindings {
    view_label: "Findings: Finding Iam Bindings"
    sql: LEFT JOIN UNNEST(${findings_log.finding__iam_bindings}) as findings__finding__iam_bindings ;;
    relationship: one_to_many
  }
  join: findings__finding__database__grantees {
    view_label: "Findings: Finding Database Grantees"
    sql: LEFT JOIN UNNEST(${findings_log.finding__database__grantees}) as findings__finding__database__grantees ;;
    relationship: one_to_many
  }
  join: findings__finding__org_policies {
    view_label: "Findings: Finding Org Policies"
    sql: LEFT JOIN UNNEST(${findings_log.finding__org_policies}) as findings__finding__org_policies ;;
    relationship: one_to_many
  }
  join: findings__finding__kubernetes__pods {
    view_label: "Findings: Finding Kubernetes Pods"
    sql: LEFT JOIN UNNEST(${findings_log.finding__kubernetes__pods}) as findings__finding__kubernetes__pods ;;
    relationship: one_to_many
  }
  join: findings__finding__security_marks {
    view_label: "Findings: Finding Security Marks"
    sql: LEFT JOIN UNNEST(${findings_log.finding__security_marks}) as findings__finding__security_marks ;;
    relationship: one_to_many
  }
  join: findings__finding__kubernetes__roles {
    view_label: "Findings: Finding Kubernetes Roles"
    sql: LEFT JOIN UNNEST(${findings_log.finding__kubernetes__roles}) as findings__finding__kubernetes__roles ;;
    relationship: one_to_many
  }
  join: findings__finding__load_balancers {
    view_label: "Findings: Finding Load Balancers"
    sql: LEFT JOIN UNNEST(${findings_log.finding__load_balancers}) as findings__finding__load_balancers ;;
    relationship: one_to_many
  }
  join: findings__finding__compliances {
    view_label: "Findings: Finding Compliances"
    sql: LEFT JOIN UNNEST(${findings_log.finding__compliances}) as findings__finding__compliances ;;
    relationship: one_to_many
  }
  join: findings__finding__connections {
    view_label: "Findings: Finding Connections"
    sql: LEFT JOIN UNNEST(${findings_log.finding__connections}) as findings__finding__connections ;;
    relationship: one_to_many
  }
  join: findings__finding__kubernetes__nodes {
    view_label: "Findings: Finding Kubernetes Nodes"
    sql: LEFT JOIN UNNEST(${findings_log.finding__kubernetes__nodes}) as findings__finding__kubernetes__nodes ;;
    relationship: one_to_many
  }
  join: findings__finding__kubernetes__objects {
    view_label: "Findings: Finding Kubernetes Objects"
    sql: LEFT JOIN UNNEST(${findings_log.finding__kubernetes__objects}) as findings__finding__kubernetes__objects ;;
    relationship: one_to_many
  }
  join: findings__finding__source_properties {
    view_label: "Findings: Finding Source Properties"
    sql: LEFT JOIN UNNEST(${findings_log.finding__source_properties}) as findings__finding__source_properties ;;
    relationship: one_to_many
  }
  join: findings__finding__contacts {
    view_label: "Findings: Finding Contacts"
    sql: LEFT JOIN UNNEST(${findings_log.finding__contacts}) as findings__finding__contacts ;;
    relationship: one_to_many
  }
  join: findings__finding__indicator__ip_addresses {
    view_label: "Findings: Finding Indicator Ip Addresses"
    sql: LEFT JOIN UNNEST(${findings_log.finding__indicator__ip_addresses}) as findings__finding__indicator__ip_addresses ;;
    relationship: one_to_many
  }
  join: findings__finding__kubernetes__bindings {
    view_label: "Findings: Finding Kubernetes Bindings"
    sql: LEFT JOIN UNNEST(${findings_log.finding__kubernetes__bindings}) as findings__finding__kubernetes__bindings ;;
    relationship: one_to_many
  }
  ...
  join: findings__resource__folders {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.resource__folders}) as findings__resource__folders ;;
    relationship: one_to_many
  }
  ...
  join: findings__finding__exfiltration__sources {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__exfiltration__sources}) as findings__finding__exfiltration__sources ;;
    relationship: one_to_many
  }
  join: findings__finding__exfiltration__targets {
    view_label: "Findings: Finding Exfiltration Targets"
    sql: LEFT JOIN UNNEST(${findings_log.finding__exfiltration__targets}) as findings__finding__exfiltration__targets ;;
    relationship: one_to_many
  }
  join: findings__finding__ip_rules__source_ip_ranges {
    view_label: "Findings: Finding Ip Rules Source Ip Ranges"
    sql: LEFT JOIN UNNEST(${findings_log.finding__ip_rules__source_ip_ranges}) as findings__finding__ip_rules__source_ip_ranges ;;
    relationship: one_to_many
  }
  join: findings__finding__ip_rules__exposed_services {
    view_label: "Findings: Finding Ip Rules Exposed Services"
    sql: LEFT JOIN UNNEST(${findings_log.finding__ip_rules__exposed_services}) as findings__finding__ip_rules__exposed_services ;;
    relationship: one_to_many
  }
  join: findings__finding__kubernetes__node_pools {
    view_label: "Findings: Finding Kubernetes Node Pools"
    sql: LEFT JOIN UNNEST(${findings_log.finding__kubernetes__node_pools}) as findings__finding__kubernetes__node_pools ;;
    relationship: one_to_many
  }
  ...
  join: findings__finding__org_policy_constraints {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__org_policy_constraints}) as findings__finding__org_policy_constraints ;;
    relationship: one_to_many
  }
  join: findings__finding__kubernetes__access_reviews {
    view_label: "Findings: Finding Kubernetes Access Reviews"
    sql: LEFT JOIN UNNEST(${findings_log.finding__kubernetes__access_reviews}) as findings__finding__kubernetes__access_reviews ;;
    relationship: one_to_many
  }
  ...
  join: findings__finding__ip_rules__destination_ip_ranges {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__ip_rules__destination_ip_ranges}) as findings__finding__ip_rules__destination_ip_ranges ;;
    relationship: one_to_many
  }
  ...
  join: findings__finding__mitre_attack__additional_tactics {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__mitre_attack__additional_tactics}) as findings__finding__mitre_attack__additional_tactics ;;
    relationship: one_to_many
  }
  join: findings__finding__mitre_attack__primary_techniques {
    view_label: "Findings: Finding Mitre Attack Primary Techniques"
    sql: LEFT JOIN UNNEST(${findings_log.finding__mitre_attack__primary_techniques}) as findings__finding__mitre_attack__primary_techniques ;;
    relationship: one_to_many
  }
  ...
  join: findings__finding__vulnerability__cve__references {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__vulnerability__cve__references}) as findings__finding__vulnerability__cve__references ;;
    relationship: one_to_many
  }
  ...
  join: findings__finding__ip_rules__denied__ip_rules {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__ip_rules__denied__ip_rules}) as findings__finding__ip_rules__denied__ip_rules ;;
    relationship: one_to_many
  }
  join: findings__finding__backup_disaster_recovery__policies {
    view_label: "Findings: Finding Backup Disaster Recovery Policies"
    sql: LEFT JOIN UNNEST(${findings_log.finding__backup_disaster_recovery__policies}) as findings__finding__backup_disaster_recovery__policies ;;
    relationship: one_to_many
  }
  join: findings__finding__mitre_attack__additional_techniques {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__mitre_attack__additional_techniques}) as findings__finding__mitre_attack__additional_techniques ;;
    relationship: one_to_many
  }
  join: findings__finding__ip_rules__allowed__ip_rules {
    view_label: "Findings: Finding Ip Rules Allowed Ip Rules"
    sql: LEFT JOIN UNNEST(${findings_log.finding__ip_rules__allowed__ip_rules}) as findings__finding__ip_rules__allowed__ip_rules ;;
    relationship: one_to_many
  }
  join: findings__finding__backup_disaster_recovery__applications {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__backup_disaster_recovery__applications}) as findings__finding__backup_disaster_recovery__applications ;;
    relationship: one_to_many
  }
  ...
  join: findings__finding__backup_disaster_recovery__policy_options {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__backup_disaster_recovery__policy_options}) as findings__finding__backup_disaster_recovery__policy_options ;;
    relationship: one_to_many
  }
  ...
  join: findings__finding__indicator__signatures {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__indicator__signatures}) as findings__finding__indicator__signatures ;;
    relationship: one_to_many
  }
  join: findings__finding__access__service_account_delegation_info {
    ...
    sql: LEFT JOIN UNNEST(${findings_log.finding__access__service_account_delegation_info}) as findings__finding__access__service_account_delegation_info ;;
    relationship: one_to_many
  }
